...
test_that("parameter estimation and moments conincide with the precomputed ones", {
  ...
  X_moments <- estimate_moments(X50)
  moments <- eval_portfolio_moments(w = rep(1/N, N), X_moments = X_moments)
  load("moments_check.RData")
  expect_equivalent(moments, moments_check)
  
  X_moments_adjusted <- estimate_moments(X50, adjust_magnitude = TRUE)
  moments_adjusted <- eval_portfolio_moments(w = rep(1/N, N), X_moments = X_moments_adjusted)
