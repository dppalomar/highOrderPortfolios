...
vignette: >
  ...
  %\VignetteKeyword{High-order portfolios}
  %\VignetteKeyword{skewness}
  %\VignetteKeyword{kurtosis}
  %\VignetteKeyword{efficient algorithm}
...
options(width=120)
# rmarkdown::render("vignettes/highOrderPortfolios.Rmd", "prettydoc::html_pretty")
```
...
sol <- design_MVSKtilting_portfolio(d, X_moments, w_init = w0, w0 = w0, 
                                    w0_moments = w0_moments, kappa = kappa, ftol = 1e-10)
...
# What is the High-order Portfolio?
## Signal model
...
- The asymmetry and heavy-tailedness of portfolio return are well captured by its third and fourth central moments, i.e., skewness and kurtosis.
- Incorporating high-order moments of the portoflio return can approximates a general expected utility function better with its Taylor series expansion truncated to the four most important order terms.
...
This package implements the algorithms designed in the paper [@zhou2020highOrderPortfolios]. The two main problem formulations covered in that paper are mean-variance-skewness-kurtosis (MVSK) and mean-variance-skewness-kurtosis tilting (MVSKT) problems.
...
- $\phi_{3}\left(\mathbf{w}\right) = \mathbf{w}^{T}\boldsymbol{\Phi}\left(\mathbf{w}\otimes\mathbf{w}\right)$ is the third central moment of portfolio return, where $\boldsymbol{\Phi} \in \mathbb{R}^{N \times N^2}$ is the coskewness matrix,
- $\phi_{4}\left(\mathbf{w}\right) = \mathbf{w}^{T}\boldsymbol{\Psi}\left(\mathbf{w}\otimes\mathbf{w}\otimes\mathbf{w}\right)$ is the third central moment of portfolio return, where $\boldsymbol{\Psi} \in \mathbb{R}^{N \times N^3}$ is the cokurtosis matrix.
...
## Estimate the High-order Comoment Matrix

Unlike the mean and covariance matrix, the coskewness and cokurtosis matrix are rarely estimated. The basic R does not provide an embedded function for estimating them. Therefore, we conclude a function `estimate_moments()` in this package to help estimate the sample coskewness and cokurtosis matrix. A simple code example is presented as follows:
```{r, echo=TRUE}
library(highOrderPortfolios)
...
```{r, echo=FALSE}
# estimate moments, align the order of magnitude
X_moments <- estimate_moments(X50, adjust_magnitude = TRUE)
```
