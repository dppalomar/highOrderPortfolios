...
csl-entry-spacing: 5em  # doesn't work
...
rbind(data.frame(idx = 1:ncol(X50), weights = sol_nonparam$w, type = "via sample moments"),
  ...
  geom_bar(color = "black", stat = "identity", position = "dodge", width = 0.8) +
...
```
or based on the parametric skew $t$ modeling:
```{r, echo=TRUE}
#TODO: Xiwen
```

We can now plot the designed MVSK tilting portfolios (both the non-parametric and parametric cases):
```{r, echo=TRUE, fig.width=9, fig.height=4, out.width="100%"}
library(ggplot2)
